...
export const metadata: Metadata = {
  title: "Top Private Universities & Colleges - College Corridor",
  description: "Browse top private universities in India. Compare rankings, NAAC grades, admissions cutoffs, and placements metrics.",
  keywords: ["private universities", "private colleges", "top private universities"]
};

export default async function PrivateUniversitiesPage() {
  const list = await prisma.institution.findMany({
    where: { published: true, ownership: { mode: "insensitive", equals: "Private" } },
    orderBy: { name: "asc" }
  });
      ...
      <div className="max-w-7xl mx-auto px-4 space-y-10">
        <div className="text-center space-y-3 max-w-xl mx-auto">
          ...
          <p className="text-slate-500 text-sm">
            Evaluate admission requirements, cutoffs, and infrastructure for leading private universities in India.
          </p>
        </div>
        ...
        {list.length === 0 ? (
          <Card className="border border-dashed py-12 text-center max-w-md mx-auto">
            <CardContent className="text-slate-400 text-sm">
              No private universities found in the database.
            </CardContent>
          </Card>
        ) : (
              ...
              <Card key={u.id} className="border-slate-200 hover:border-indigo-600/30 transition-all hover:shadow-md bg-white flex flex-col justify-between">
                ...
                <CardContent className="p-5 space-y-4">
                  <div className="flex gap-2">
                    <Badge className="bg-indigo-50 text-indigo-700 border-indigo-200">Private University</Badge>
                    {u.isPartner && <Badge className="bg-emerald-50 text-emerald-700 border border-emerald-200">Official Partner</Badge>}
                  </div>
                  <p className="text-xs text-slate-500 line-clamp-2">
                    {u.description || "Admissions cutoffs details, rankings, and placement statistics for the upcoming academic year."}
                  </p>
